...
# Dataset Name: Sex Offenders -- Georgia
# Task #:       53
# Created:      November 2021

# ruby mlc.rb --tool="clean::ga::sex_offenders"
# ruby mlc.rb --tool="clean::ga::sex_offenders" --mode='name'
# ruby mlc.rb --tool="clean::ga::sex_offenders" --mode='city'
# ruby mlc.rb --tool="clean::ga::sex_offenders" --mode='crime'

def execute(options = {})
  route = C::Mysql.on(DB01, 'sex_offenders')
  table_description = {
    name: {
      raw_table: 'Georgia',
      clean_table: 'Georgia__names_clean',
      raw_column: 'name',
      clean_column: 'name_clean',
    },
    city: {
      raw_table: 'Georgia',
      clean_table: 'Georgia__cities_clean',
      state_column: 'state',
      raw_column: 'city',
    ...
    crime: {
      raw_table: 'Georgia',
      clean_table: 'Georgia__crimes_clean',
      raw_column: 'crime',
      clean_column: 'crime_clean',
      # local_connection: true
    }
  }
  ...
  when :crime
    crimes_cleaning(table_info, where_part, route)
  else
...
def message_to_slack(message)
  Slack::Web::Client.new.chat_postMessage(
    channel: 'UKLB1JGDN',
    text: "*[CLEANING #53] Sex Offenders -- Georgia* \n>#{message}",
    as_user: true
  )
end
...
def clean_crimes(table_info, route)
  ...
  crimes_list.each do |item|
    ...
    clean_description.sub!(/\b(lasc(iv)?|lasciviou(is)?)\b/, 'lascivious')
    ...
    clean_description.gsub!(/\byoa\b/, 'years of age')
    ...
    prepared_description[table_info[:clean_column]] = clean_description.squeeze(' ')
