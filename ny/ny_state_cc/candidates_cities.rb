...
def candidates_cities_query
  <<~SQL
    SELECT DISTINCT
      state AS st,
      city
    FROM ny_campaign_finance_candidates
...
def candidates_cities_cleaning
  begin
    db01 = C::Mysql.on(DB01, 'usa_raw')

    cities_to_clear = db01.query(candidates_cities_query).to_a
      ...
      Thread.new do
        ...
        loop do
          ...
          begin
            ...
            update_query = <<~SQL
              UPDATE ny_campaign_finance_candidates
