...
# Dataset Name: UT Lawyer Status
# Task #:       80
# Scrape Task:  https://lokic.locallabs.com/scrape_tasks/147
# Created:      August 2022

# ruby mlc.rb --tool="clean::ut::ut_lawyer_status"
# ruby mlc.rb --tool="clean::ut::ut_lawyer_status" --mode='city'
# ruby mlc.rb --tool="clean::ut::ut_lawyer_status" --mode='zip'
# ruby mlc.rb --tool="clean::ut::ut_lawyer_status" --mode='status'
# ruby mlc.rb --tool="clean::ut::ut_lawyer_status" --mode='law_firm_name'
...
def execute(options = {})
  ...
  table_description = {
    city: {
      raw_table: 'utah',
      clean_table: 'utah__cities_clean',
    ...
    status: {
      raw_table: 'utah',
      clean_table: 'utah_lawyer_status_clean',
      raw_status_column: 'status',
      raw_type_column: 'type',
      clean_status_column: 'status_clean',
    },
    law_firm_name: {
      raw_table: 'utah',
      clean_table: 'utah__law_firm_name_clean',
...
def message_to_slack(message, type = '')
  ...
  Slack::Web::Client.new.chat_postMessage(
    channel: 'UKLB1JGDN',
    text: "*[CLEANING #80] UT Lawyer Status* \n>>#{type} #{message}",
    as_user: true
  )
end
...
def fill_status_table(table_info, where_part, route)
  ...
  query = <<~SQL
    ...
    WHERE cl.id IS NULL
      AND deleted=0
      AND r.#{table_info[:raw_status_column]} IS NOT NULL
      AND r.#{table_info[:raw_type_column]} IS NOT NULL
      #{"AND #{where_part}" if where_part}
...
def check_status(table_info, route)
  query = <<~SQL
    SELECT #{table_info[:raw_status_column]}, #{table_info[:raw_type_column]}
    FROM #{table_info[:clean_table]}
    WHERE #{table_info[:clean_status_column]} IS NULL;
  SQL
  ...
  law_firm_name_list.each do |row|
    ...
    if result_name.size <= 2 ||
       ...
       ((result_name.match?(/^city of /i) || result_name.match?(/\scity$/i)) &&
         !result_name.match?(/\b(Authority|Corporation|Court|Department|Diocese|FBI|Office)\b/i))
      skip_it = 1
    end
    result_name = MiniLokiC::Formatize::Cleaner.org_clean(result_name.gsub('|', ' '))
    result_name = add_point(result_name)
    clean_name[table_info[:clean_column]] = result_name.sub(/\bAt\b/, 'at').sub(/\bOf\b/, 'of')
